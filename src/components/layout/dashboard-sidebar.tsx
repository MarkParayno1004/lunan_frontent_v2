...
const navByRole: Record<SidebarRole, NavItem[]> = {
  ...
  PATIENT: [
    { href: "/dashboard/patient", label: "My Plan", icon: ClipboardList },
  ...
  return (
    <aside className="w-full rounded-2xl border border-white/40 bg-white/60 p-4 shadow-sm backdrop-blur md:w-64">
      <div className="mb-4 px-2">
        <p className="text-xs font-medium uppercase tracking-wide text-zinc-500">Lunan</p>
        <p className="text-sm font-semibold text-zinc-900">{normalizedRole.toLowerCase()} portal</p>
      </div>
